...
class TestCase(BaseModel):
    ...
    expected_tool: str | None = Field(None, description="Expected tool to use")
...
class ComparisonRequest(BaseModel):
    ...
    test_cases: list[TestCase] | None = Field(
        None, description="Custom test cases (uses defaults if not provided)"
    )
    verbose: bool = Field(
        default=False, description="Include detailed execution traces"
    )
    ...
    model_config = {
        "json_schema_extra": {
            "example": {
                ...
                "avg_steps": 3.2,
                "tool_usage": {"pink_floyd_database": 8, "currency_converter": 2},
            }
        }
    }
...
class ComparisonResult(BaseModel):
    ...
    winner: str | None = Field(None, description="Best performing model")
        ...
        "json_schema_extra": {
            "example": {
                ...
                "results": {
                    ...
                    "gpt-4o": {"success": True, "execution_time": 3.5, "cost": 0.0089},
                },
                "winner": "gpt-4o-mini",
            }
        }
...
class ComparisonResponse(BaseModel):
    ...
    summary: dict[str, ModelMetricsSummary] = Field(
        ..., description="Summary per model"
    )
    detailed_results: list[ComparisonResult] | None = Field(
        None, description="Detailed results (if verbose=True)"
    )
    timestamp: str = Field(..., description="Comparison timestamp")
    total_duration: float = Field(
        ..., description="Total comparison duration (seconds)"
    )
