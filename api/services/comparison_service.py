...
class ComparisonService:
    ...
    async def run_comparison(
        ...
        try:
            logger.info(f"Starting comparison of {len(models)} models: {', '.join(models)}")
            ...
            for model_name in models:
                if model_name in comparison_data:
                    ...
                    summary[model_name] = {
                        ...
                        "avg_execution_time": metrics.get("execution_time", {}).get("mean", 0),
                    ...
                    for model_name in models:
                        if model_name in results and i < len(results[model_name]):
                            ...
                            result_entry["results"][model_name] = {
                                "success": "Error" not in model_result.get("answer", ""),
                                "execution_time": model_result.get("metrics", {}).get("execution_time_seconds", 0),
                                "cost": model_result.get("metrics", {}).get("estimated_cost_usd", 0),
